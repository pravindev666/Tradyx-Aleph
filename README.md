...
## 🏛️ PROLOGUE: THE PHILOSOPHY OF EVOLUTION
...
## 🏗️ CHAPTER 1: SENTIENT 2.0 - THE ENSEMBLE ERA
...
```mermaid
graph TD
    Data[Market Data: OHLCV] --> FE[Feature Engineer: RSI, SMA]
    FE --> XGB[XGBoost: Gradient Boosting]
    FE --> LGB[LightGBM: Fast Boosting]
    FE --> RF[Random Forest: Bagging]
    XGB & LGB & RF --> Vote{Democratic Vote}
    Vote -->|2/3 Match| Verdict[FINAL VERDICT]
    Vote -->|Conflict| Neutral[STAY NEUTRAL]
```
...
## 🛡️ CHAPTER 2: SENTIENT 2.1 - THE DEFENSE ERA
...
```mermaid
graph TD
    Data --> FE --> Ensemble[ML Ensemble]
    Data --> ISO[Isolation Forest: Anomaly Radar]
    Ensemble --> Filter{Chaos Check}
    ISO -->|Anomaly Detected| Veto[VETO: SHUTDOWN]
    ISO -->|Normal| Filter
    Filter -->|Pass| ELI5[ELI5: Feature Narrative]
    ELI5 --> Verdict[AUDITED VERDICT]
```
...
```mermaid
graph TD
    subgraph "The Strategy Loop"
        State[Market State: 13 Pillars] --> PPO[PPO Agent: Policy Optimization]
        PPO --> Action[Action: Buy, Sell, Hold]
        Action --> Reward[Reward: Profit/Loss]
        Reward -->|Feedback| PPO
    end
    ML_Verdict[ML Ensemble: Probabilities] --> PPO
    PPO --> Strategy[SCALE IN / SCALE OUT]
```
...
```mermaid
graph TD
    Data --> Polars["Polars: Rust-Engine (10x Speed)"]
    Polars --> GARCH[GARCH: Volatility Regime Detection]
    Polars --> Darts[Darts: Seasonal Cycle DNA]
    GARCH & Darts --> Brain[Grandmaster PPO Agent]
...
## ⚖️ CHAPTER 6: SIDE-BY-SIDE COMPARATIVE MATRIX
...
```mermaid
graph LR
    subgraph "Standard ML (Legacy)"
        ID[Input Data] --> PF[Predefined Formula: RSI/SMA]
        PF --> ML[Model Prediction]
        ML --> Result[Up/Down]
    end

    subgraph "Alien Math (The Singularity)"
        ID2[Input Data] --> GE2[Genetic Discovery]
        GE2 -->|Evolution| AM[Alien Formula: Discovering New Physics]
        AM -->|Audit| A17_2[The Auditor]
        A17_2 --> Result2[Strategic Sovereignty]
    end
```
...
```mermaid
graph TD
    Birth[1. Genetic Discovery] --> Training[2. Training: 2000-2020]
    Training --> Test[3. OUT-OF-SAMPLE TEST: 2021-2025]
    Test -->|Fail| Deletion[DEATH: Formula Extinct]
    Test -->|Pass| Ranking[ACCURACY RANKING]
    Ranking --> Council[ADMISSION TO ELITE 12]
```
